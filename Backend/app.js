...
app.get("/solvedquestions", async (req, res) => {
  let solvedquestion = await fetch(` https://codeforces.com/api/user.status?handle=${req.body.id}`);
  solvedquestion=await question.json();
  console.log(solvedquestion.problems);
  res.status(200).send(solvedquestion);
});

app.get("/getquestions",async(req,res)=>{
    let questions=await fetch("https://codeforces.com/api/problemset.problems?tags=implementation");
    ...
    problems.forEach((element,index) => {
        let temp={name:element.name,tags:element.tags,submissions:stats[index].solvedCount}
    ...
    const dk={q800,q900,q1000,q1100,q1200,q1300,q1400,q1500,q1600};
